...
class lstm(nn.Module):
    def __init__(self, opt):
        ...
        self.fc1 = nn.Sequential(
            nn.BatchNorm1d(self.source_dim+self.hidden_dim),
        ...
        self.main = nn.Sequential(
            # nn.Dropout(),
            nn.Linear(self.source_dim+self.hidden_dim, self.tagset_size),
    ...
    def forward(self, batch):
        ...
        hg_a = torch.cat([hgs_mean,torch.mean(lstm_out,0)],dim=1)
        # hg_a = torch.cat([hgs_mean, lstm_out[-1]], dim=1)

        lstm_out_mean_fc = self.fc1(hg_a)
        ...
        return lstm_out_train, lstm_out_mean_fc