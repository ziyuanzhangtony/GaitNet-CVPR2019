...
class encoder(nn.Module):
    def __init__(self, opt):
        ...
        # self.fa_branch = nn.Sequential(
        #     nn.LeakyReLU(0.2),
        #     nn.Linear(self.em_dim,self.opt.fa_dim),
        #     nn.BatchNorm1d(self.opt.fa_dim),
        # )
        #
        # self.fg_branch = nn.Sequential(
        #     nn.LeakyReLU(0.2),
        #     nn.Linear(self.em_dim, self.opt.fg_dim),
        #     nn.BatchNorm1d(self.opt.fg_dim),
        # )

        self.fgs_clf = nn.Sequential(
            nn.LeakyReLU(0.2),
            nn.Linear(self.opt.fg_dim//2, self.opt.num_train),
    ...
    def forward(self, input):
        ...
        # fa = self.fa_branch(embedding)
        # fg = self.fg_branch(embedding)
        fa, fg = torch.split(embedding, [self.opt.fa_dim, self.opt.fg_dim], dim=1)
        fgs,fgd = torch.split(fg, [self.opt.fg_dim//2, self.opt.fg_dim//2], dim=1)
...
class decoder(nn.Module):
    ...
    def forward(self, fa, fg):
        hidden = torch.cat([fa, fg[0],fg[1]], 1).view(-1, self.opt.em_dim)
...
class lstm(nn.Module):
    def __init__(self, opt):
        ...
        self.source_dim = opt.fg_dim//2
        self.hidden_dim = opt.lstm_hidden_dim-opt.fg_dim//2
    ...
    def forward(self, batch):
        ...
        hgs_mean = torch.mean(hgs,0).view(-1,self.source_dim)
        # hgs_mean = torch.max(hgs, 0)[0]
        hgd = torch.stack([i[1] for i in batch])
        # hgd_mean = torch.mean(hgd,0).view(-1,self.source_dim)


        lstm_out, _ = self.lstm(hgd)
        ...
        # hg_a = torch.cat([hgs_mean,lstm_out_mean],dim=1)
        # hg_a2 = torch.cat([hgs_mean, hgd_mean], dim=1)
        # hg_a2 = (0.1*hgs_mean+0.9*hgd_mean)/2
        # hg_a = torch.cat([hgs_mean, lstm_out[-1]], dim=1)

        lstm_out_mean_fc = self.fc1(lstm_out_mean)
        fg_final = torch.cat([lstm_out_mean_fc, hgs_mean], dim=1)
        # lstm_out_test = torch.mean(lstm_out.view(num_frames,-1,self.hidden_dim),0)

        lstm_out_train = self.main(lstm_out_mean_fc).view(-1, self.tagset_size)

        return lstm_out_train, fg_final