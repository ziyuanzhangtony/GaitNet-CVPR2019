...
parser.add_argument('--signature', default='PAMI-74fortraining-NEW_MRCNN-NM_CL-AVG_LSTM_INI-64_128-max_pooling_3-256hidden-00_00-rate_decay_all-fgs_fgd-64d-test_len_50-with_factor-sperate_hgs_hgd-l1')
...
if loading_model_path:
    checkpoint = torch.load(loading_model_path,'cuda:0')
...
def train_main(Xn, Xc, l, X2):
    ...
    for i in range(0, len(Xn)):
        ...
        self_rec_loss += l1_loss(xmx1_, xmx1)
        # ------------------------------------------------------------------
        xn1, xc1 = Xn[i], Xc[i]
        _, hgn1 = netE(xn1)
        _, hgc1 = netE(xc1)
...
plot_anology(test_analogy_row, test_analogy_col,0,[netE,netD],opt)
plot_tsne(,0,[netE,netD],opt)
