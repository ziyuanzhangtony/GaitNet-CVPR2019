...
parser.add_argument('--signature', default='100fortraining-new-benchmark-average-only')
...
if loading_model_path:
    checkpoint = torch.load(loading_model_path,map_location={'cuda:1': 'cuda:0'})
...
#################################################################################################################
def visualization_result(eval_data,groundtruth_predicted):
    correct = torch.ones([3, 64, 32])
    wrong = torch.zeros([3, 64, 32])
    result = []
    for gt, pred in groundtruth_predicted:
        if gt[0] == pred:
            result.append(correct)
        else:
            result.append(wrong)
    result = torch.stack(result)
    gallery = eval_data[0][0][0]
    probe = eval_data[1][0][0]
    perm = [i[1] for i in groundtruth_predicted]
    predict = eval_data[1][0][0][perm]
    whole = torch.cat([result, gallery, probe, predict], dim=0)
    torchvision.utils.save_image(whole, 'visual.png',24)
...
with torch.no_grad():
    ...
    scores_cmc_cl_test, groundtruth_predicted = eval_cmc(test_eval[0], test_eval[1], [netE, lstm], opt, [90], [90], True)
    visualization_result(test_eval, groundtruth_predicted)
