...
data, label = test_data.get_tsne_format()
# for i in label:
#     print(i)
netE.eval()
lstm.eval()
with torch.no_grad():
    ...
    fg_pb = torch.stack([netE(data[i].cuda())[1] for i in range(len(data))])
    # fg_pb = lstm(fg_pb)[2]
    # fg_pb = torch.stack(fg_pb, 0).view(len(fg_pb), -1, opt.fg_dim)
    # pb_vec = lstm(fg_pb)[1].detach().cpu().numpy()

    # fgs_pb = [netE(fg_pb[i].cuda())[0][:, :128] for i in range(len(fg_pb))]
    # fgs_pb = torch.stack(fgs_pb, 0).mean(dim=0).detach().cpu().numpy()
    # features = fg_pb.mean(dim=0)
    X = fg_pb.view(N*length*2,feature_len)
    # visual = data.view([5000,3,64,32])
    ...
    # plt.show()
    plt.savefig('tsne_fgd.png')

    # writer.add_embedding(features, metadata=label,global_step=1)
