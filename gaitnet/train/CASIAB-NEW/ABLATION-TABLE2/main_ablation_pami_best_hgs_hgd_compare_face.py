...
with torch.no_grad():
    for i in range(1, 10 + 1):
        percent = int(50 * i / 10)
        partial_test = test_eval[1][0][:,:percent,:,:,:]
        scores_cmc_cl_test, _ = eval_cmc_two(test_eval[0], [partial_test], [netE, lstm], opt, [90], [90], True,1,2)
        scores.append(scores_cmc_cl_test[0])
