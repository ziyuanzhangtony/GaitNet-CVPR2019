...
from utils.modules.modules_cb_pami_tab5 import *
...
train_structure = {
    'clip1': ([36,54,72,90,108,126,144],
              {
                  'cl':list(range(1, 2 + 1)),
               'bg':list(range(1, 2 + 1))}
              ),
    'clip2': ([36,54,72,90,108,126,144],
          {
              'cl':list(range(1, 2 + 1)),
           'bg':list(range(1, 2 + 1))}
          ),
...
# test_structure = {
#     'gallery': ([36,54,72,90,108,126,144], ['cl'], [1]),
#     'probe': ([36,54,72,90,108,126,144], ['cl'], [2]),
# }
...
with torch.no_grad():
    scores_cmc_cl_test, _ = eval_cmc_two(test_eval[0], test_eval[1], [netE, lstm], opt, test_structure['gallery'][0],
                                         test_structure['probe'][0], False, 1, 1)
    test_acc_mean = sum(scores_cmc_cl_test) / len(scores_cmc_cl_test)
    print(test_acc_mean)
...
#             netE.eval()
#             lstm.eval()
#             # scores_cmc_cl_train, _ = eval_cmc_two(train_eval[0], train_eval[1],[netE, lstm], opt, test_structure['gallery'][0], test_structure['probe'][0], False,4,2)
#             scores_cmc_cl_test, _ = eval_cmc_two(test_eval[0], test_eval[1], [netE, lstm], opt, test_structure['gallery'][0], test_structure['probe'][0], False,1,1)
...
#
#             if itr <= 8000:
#                 if test_acc_mean >= max_test_acc:
#                     torch.save({
#                         'netD': netD.state_dict(),
#                         'netE': netE.state_dict(),
#                         'lstm': lstm.state_dict(),
#                     },
#                         '%s/modules/%s/%d_%d.pickle' % (opt.savedir, opt.signature, itr, int(test_acc_mean*1000)), )
#                     max_test_acc = test_acc_mean
#             else:
...
#                         opt.savedir, opt.signature, itr, test_acc_mean * 1000))
#                     max_test_acc = test_acc_mean
#
#             # plot_anology(test_analogy_row, test_analogy_col,0,[netE,netD],opt, writer)
#
