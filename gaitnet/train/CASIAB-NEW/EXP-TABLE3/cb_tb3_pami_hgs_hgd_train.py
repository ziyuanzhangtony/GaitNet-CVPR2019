...
from utils.modules.modules_cb_pami_tab3 import *
...
test_structure = {
    'gallery': ([90], ['nm'], [1]),
    'probe': (list(range(0,180+1,18)), ['nm'], [2]),
}
...
while True:
    ...
    if itr % 10 == 0:
        with torch.no_grad():
            ...
            netE.eval()
            lstm.eval()
            scores_cmc_cl_train, _ = eval_cmc_two(train_eval[0], train_eval[1],[netE, lstm], opt, test_structure['gallery'][0], test_structure['probe'][0], False,1,1)
