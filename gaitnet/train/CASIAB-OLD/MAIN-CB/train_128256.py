...
class decoder(nn.Module):
    def __init__(self, nc=3):
        ...
        self.trans = nn.Sequential(
            nn.Linear(self.em_dim, nf * 32 * 2 * 4),
            nn.BatchNorm1d(nf * 32 * 2 * 4),
        )
...
parser.add_argument('--image_height', type=int, default=256, help='the height of the input image to network')
parser.add_argument('--image_width', type=int, default=128, help='the width of the input image to network')
...
def plot_anology(data1, data2, epoch):
    anology1 = make_analogy_inter_subs(data2)
    ...
    anology1 = make_analogy_intal_subs(data2, data2, 8)
    anology2 = make_analogy_intal_subs(data2, data2, 9)
    all = torch.cat([anology1, anology2], dim=1)
    all = adjust_white_balance(all.detach())
