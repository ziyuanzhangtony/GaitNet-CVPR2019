...
class CASIAB(object):
    ...
    def __getitem__(self, index):
        ...
        def random_chose():
            ...
            vi_idx1 = np.random.choice(self.glr_views)
            vi_idx2 = np.random.choice(self.glr_views)
            return si_idx, vi_idx1,vi_idx2,labels
        ...
        return imgs_nm, imgs_cl,imgs_mx, labels, int(vi_mx/18)
...
class encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        fa,fv,fg = torch.split(embedding, [opt.ha_dim, opt.hv_dim, opt.hg_dim], dim=1)
        return fa,fv,fg

class classification(nn.Module):
    def __init__(self):
        super(classification, self).__init__()
        self.ha_out_dim = opt.num_train*2
        self.hv_out_dim = 11
        ...
        self.main_ha = nn.Sequential(
            ...
            nn.Linear(opt.ha_dim*4, self.ha_out_dim),
            nn.BatchNorm1d(self.ha_out_dim)
        )

        self.main_hv = nn.Sequential(
            nn.Linear(opt.hv_dim, opt.hv_dim * 2),
            nn.BatchNorm1d(opt.hv_dim * 2),
            nn.LeakyReLU(),

            nn.Linear(opt.hv_dim * 2, opt.hv_dim * 4),
            nn.BatchNorm1d(opt.hv_dim * 4),
            nn.LeakyReLU(),

            nn.Linear(opt.hv_dim * 4, self.hv_out_dim),
            nn.BatchNorm1d(self.hv_out_dim)
        )
    ...
    def forward(self, input,branch):
        if branch == 0: # for fa
            embedding = self.main_ha(input).view(-1,self.ha_out_dim)
        elif branch == 1: # for fv
            embedding = self.main_hv(input).view(-1, self.hv_out_dim)
        # elif branch == 2:
...
class decoder(nn.Module):
    ...
    def forward(self,ha,hv,hg):
        return self.main(torch.cat([ha,hv,hg], 1).view(-1,self.em_dim,1,1))
...
parser.add_argument('--ha_dim', type=int, default=256, help='size of the appearance vector')
parser.add_argument('--hv_dim', type=int, default=32, help='size of the appearance vector')
...
def make_analogy(x):
    ...
    gene = [netD(torch.stack([h_as[i]] * 8).cuda(),torch.stack([h_vs[i]] * 8).cuda(),h_gs) for i in range(len(h_as))]
...
def train_main(Xn,Xc,Xmx,l_id,l_view):
    l_cl = l_id + opt.num_train
    ...
    for i in range(5,len(Xn)):
        ...
        hamx0, hvmx0, hgmx0= netE(xmx0)
        hamx1, hvmx1, hgmx1= netE(xmx1)
        # ------------------------------------------------------------------
        xmx1_ = netD(hamx0,hvmx0,hgmx1)
    ...
    has = torch.stack(has)
    has = torch.mean(has,0)
    has_out = clfer(has,0)
    loss_out_has = cse_loss(has_out, l_id) / 20

    hvs = torch.stack(hvs)
    hvs = torch.mean(hvs, 0)
    hvs_out = clfer(hvs, 1)
    loss_out_hvs = cse_loss(hvs_out, l_view) / 20

    hgs_n = torch.stack(hgs_n)
    hgs_n = torch.mean(hgs_n, 0)
    hgs_c = torch.stack(hgs_c)
    hgs_c = torch.mean(hgs_c, 0)
    loss_out_hgsm = mse_loss(hgs_n, hgs_c) / 100

    loss = loss_out_has+self_rec_loss+loss_out_hgsm+loss_out_hvs
    loss.backward()
    optimizerE.step()
    optimizerD.step()
    optimizerClfer.step()

    return [loss_out_has.data.cpu().numpy(),
            loss_out_hgsm.data.cpu().numpy(),
            loss_out_hvs.data.cpu().numpy()]
...
while True:
    ...
    im_nm, im_cl,im_mx,lb_id,lb_view = next(training_batch_generator1)

    losses1 = train_main(im_nm, im_cl,im_mx,lb_id,lb_view)
    write_tfboard(losses1,itr,name='EDLoss')

    losses3 = train_lstm(im_nm,im_cl,im_mx,lb_id)
    ...
    if itr % 5 == 0:
        ...
        im_nm_, im_cl_, im_mx_, _,_ = next(testing_batch_generator)
