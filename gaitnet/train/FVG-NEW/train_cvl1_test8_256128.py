...
parser.add_argument('--im_height', type=int, default=256, help='the height of the input image to network')
parser.add_argument('--im_width', type=int, default=128, help='the width of the input image to network')
...
class encoder(nn.Module):
    def __init__(self, nc=3):
        ...
        self.main = nn.Sequential(
            ...
            dcgan_conv(nf * 8, nf * 16),
            vgg_layer(nf * 16, nf * 16),

            dcgan_conv(nf * 16, nf * 32),
            vgg_layer(nf * 32, nf * 32),
        )

        self.flatten = nn.Sequential(
            nn.Linear(nf * 32 * 2 * 4,self.em_dim),
            nn.BatchNorm1d(self.em_dim),
        )

    def forward(self, input):
        embedding = self.main(input).view(-1, 2048 * 2 * 4)
...
class decoder(nn.Module):
    def __init__(self, nc=3):
        ...
        self.main = nn.Sequential(
            ...
            dcgan_upconv(nf * 32, nf * 16),
            vgg_layer(nf * 16, nf * 16),

            dcgan_upconv(nf * 16, nf * 8),
            vgg_layer(nf * 8, nf * 8),
    ...
    def forward(self, fa, fg):
        ...
        small = self.trans(hidden).view(-1, 64 * 32, 4, 2)
        img = self.main(small)
        return img
...
# optimizerE = optim.Adam(netE.parameters(), lr=opt.lr, betas=(0.9, 0.999), weight_decay=0.00001)
# optimizerD = optim.Adam(netD.parameters(), lr=opt.lr, betas=(0.9, 0.999), weight_decay=0.00001)
# optimizerLstm = optim.Adam(lstm.parameters(), lr=opt.lr, betas=(0.9, 0.999), weight_decay=0.00001)
...
def make_analogy_intal_subs(nm, cl, idx):
    ...
    none = torch.zeros([1, 3, 256, 128]).cuda()
...
if not debug_mode:
    ...
    with torch.no_grad():
        ...
        netE.eval()
        lstm.eval()
        eval_WS = eval_lstm_roc(proto_WS[0].cuda(), proto_WS[1].cuda(), proto_WS[2], 90, [netE, lstm], opt)
        print(eval_WS)
        # write_tfboard(eval_WS[:2], itr, name='WS')
        eval_CB = eval_lstm_roc(proto_CB[0].cuda(), proto_CB[1].cuda(), proto_CB[2], 90, [netE, lstm], opt)
        print(eval_CB)
        # write_tfboard(eval_CB[:2], itr, name='CB')
        eval_CL = eval_lstm_roc(proto_CL[0].cuda(), proto_CL[1].cuda(), proto_CL[2], 90, [netE, lstm], opt)
        print(eval_CL)
        # write_tfboard(eval_CL[:2], itr, name='CL')
        # writer.close()
        # batch_cond1, batch_cond2, _ = next(testing_batch_generator)
