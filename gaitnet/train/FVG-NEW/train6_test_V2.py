...
2019-05-21 16:47:15.498204, 15 FPS
"""
...
class FVG(object):
    ...
    def __init__(self,
        ...
        self.train_subjects = subjects  # 136 subjects 60%
        self.test_subjects = subjects  # 90 subjects 40%
...
class FVG_test(object):
    ...
    def get_eval_data(self,
        ...
        def read_frames_(si_idx, vi_idx):
            ...
            video_in = os.path.join(self.data_root, '%02d_%02d' % (si_idx, vi_idx))
            files = sorted(os.listdir(video_in))
        ...
        test_data_glr = pad_sequences(test_data_glr, maxlen=100, dtype='float32', padding='post')
        test_data_prb = pad_sequences(test_data_prb, maxlen=100, dtype='float32', padding='post')
    ...
    def __getitem__(self, index):
        ...
        session_dir1, vi1 = random_vi_idx(si)
        session_dir2, vi2 = random_vi_idx(si)
        video_pth1 = os.path.join(self.data_root, '%02d_%02d' % (si, vi1))
        video_pth2 = os.path.join(self.data_root, '%02d_%02d' % (si, vi2))
        start1, end1, files1 = random_length(video_pth1, si, self.sequence_len)
        start2, end2, files2 = random_length(video_pth2, si, self.sequence_len)
...
parser.add_argument('--siter', type=int, default=49000, help='number of itr to start with')
parser.add_argument('--lr', default=0.00001, type=float, help='learning rate')
parser.add_argument('--data_root',
                    default='/home/tony/Research/FVG-NEW-SEG-PNG-V2',
...
test_data = FVG_test(
    train=False,
    data_root="/media/tony/ESD-USB/esting/mrcnn_v0/",
    sequence_len=opt.max_step,
)
...
def eval_lstm_roc(glr, prb, gt, n_test=7):
    def calculate_cosine_similarity(a, b):
        score = 1 - spatial.distance.cosine(a, b)
        return score
...
def train_main(Xn, Xc, Xmx, l):
    ...
    gait_sim_loss = mse_loss(hgs_n, hgs_c) / 100
...
#################################################################################################################
# FUN TRAINING TIME !
proto1 = test_data.get_eval_data(False, 1, [2])
...
while True:
    ...
    if itr % 50 == 0:
        ...
        print(scores1)
        # write_tfboard([scores1], itr, name='ROC')
