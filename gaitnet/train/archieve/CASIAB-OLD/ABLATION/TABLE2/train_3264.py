...
class lstm(nn.Module):
    ...
    def forward(self, batch):
        ...
        lstm_out_train = self.main(lstm_out_test).view(-1, self.target_dim)
...
parser.add_argument('--data_root',
                    default='/media/tony/44E3B4EB22716E02/GaitNet-Pytorch/DATASET/CASIAB-OLD-RGB-BS/',
...
time_now = "2019-04-18 11:05:14.124762"
...
netE = encoder()
netD = decoder()
lstm = lstm(hidden_dim=128, target_dim=opt.num_train)
...
def eval_lstm_cmc(glr, prb):
    glr = glr[0]
    glr = glr.transpose(0,1)
    prb = prb[0]
    prb = prb.transpose(0,1)

    pb_vecs = []
    gr_vecs = []
    for pb in prb:
        fg_pb = [netE(pb[i:i+1].cuda())[1].detach() for i in range(len(pb)-1)]
        fg_pb = torch.stack(fg_pb, 0).view(len(fg_pb), -1, opt.hg_dim)
        pb_vec = lstm(fg_pb)[1].detach().cpu().numpy()
        pb_vecs.append(pb_vec)

    for gr in glr:
        fg_gr = [netE(gr[i:i+1].cuda())[1].detach() for i in range(len(gr)-1)]
    ...
    for pb_idx, pv in enumerate(pb_vecs):
        id_range = list(range(pb_idx, pb_idx + 1))
        score= calculate_identication_rate_single(gr_vecs, pv, id_range)[0]
        scores_all.append(score)
    return [sum(scores_all)/len(scores_all)]
...
def train_main(Xn, Xc, Xmx, l):
    ...
    gait_sim_loss = mse_loss(hgs_n, hgs_c)
...
def train_lstm(x_mx, l):
    ...
    for i in range(0, len(x_mx)):
        ...
        id_incre_loss += cse_loss(lstm_out_mx, Variable(l))
