...
parser.add_argument('--siter', type=int, default=2400, help='number of itr to start with')
...
parser.add_argument('--max_step', type=int, default=60, help='maximum distance between frames')
...
def return_clip(fg1,fg2,length):
    end_=length
    minn = calculate_cosine_similarity_multidim(fg1, fg2[0:length])
    for end in range(length, len(fg2)):
        start = end-length
        fg2_clip = fg2[start:end]
        if calculate_cosine_similarity_multidim(fg1, fg2_clip) < minn:
            end_ = end
            minn = calculate_cosine_similarity_multidim(fg1, fg2_clip)
...
def alignment_and_clip(clip1, clip2, length):
    # netE.eval()
    clip1_ = clip1[:, :length, :, :, :]
    clip1_fg = [netE(clip1_[i].cuda())[1].detach().cpu().numpy() for i in range(len(clip1_))]
    clip2_fg = [netE(clip2[i].cuda())[1].detach().cpu().numpy() for i in range(len(clip2))]
    clip2_=[]
