...
class FVG(object):
    ...
    def __init__(self,
        ...
        # self.train_subjects = subjects[:10]  # 136 subjects
        # self.test_subjects = subjects[:10]  # 136 subjects
        self.train_subjects = subjects[:136] #136 subjects
        self.test_subjects = subjects[136:] #90 subjects
    ...
    def get_lstm_test_data(self):
        def read_frames_(si_idx, vi_idx):
            video_in = os.path.join(self.data_root, 'SEG_SKIP/%03d_%02d' % (si_idx, vi_idx))
            files = os.listdir(video_in)
            files = sorted(files)
        ...
        for i,id in enumerate(self.test_subjects):
            for j in range(1,12+1):
                if j == 2:
                    test_data_glr.append(self.read_frames_(id,j))
                else:
                    test_data_prb.append(self.read_frames_(id,j))
            print(i)
    ...
    def __getitem__(self, index):
        ...
        # joints = np.zeros([self.sequence_len, 28], np.float32)
...
parser.add_argument('--siter', type=int, default=26000, help='number of itr to start with')
...
# time_now = '2018-11-21 23:43:54.025974'
time_now = '2018-11-22 12:25:17.606960'
...
def make_analogy(x,step):
    ...
    x_gs = torch.stack([i for i in [
        x[1][rand_step()], x[14][rand_step()], x[7][rand_step()], x[9][rand_step()],torch.zeros([3, 64, 64]).cuda()
                                    ]]).cuda()
    h_gs = netE(x_gs)[1]
    h_gs[4,:] = 0
    # h_gs = torch.zeros(8,20).cuda()

    x_as = torch.stack([x[i][0] for i in [2,8,15]]).cuda()
    x_as = torch.cat([x_as,torch.zeros([1,3, 64, 64]).cuda()]).cuda()
...
def plot_anology(train,test,epoch):
    for i in range(opt.max_step):
        train_anology = make_analogy(train,i)
        # test_anology = make_analogy(test)
        # all = torch.cat([train_anology,test_anology],dim=1)
        # writer.add_image('Image', train_anology, epoch)
        fname = '%s/analogy/%s/%d.png' % (opt.savedir,opt.signature,i)
        save_image(train_anology, fname, 9)


def eval_lstm_roc(glr, prb, n_test  = 90):
    ...
    hp_prb = torch.stack(hp_prb, 0).view(len(hp_prb), n_test * 11, opt.hg_dim)
    prb_vec = lstm(hp_prb)[1].detach().cpu().numpy()
    # prb_vec = torch.mean(hp_prb, 0).detach().cpu().numpy()

    obj_arr = np.zeros((n_test * 11, n_test), dtype=np.float32)
    for i in range(n_test):
        for j in range(n_test * 11):
            cs = calculate_cosine_similarity(glr_vec[i:i + 1, :],
                                             prb_vec[j:j + 1, :])
            obj_arr[j, i] = cs
    fpr, tpr,roc_auc = process_confusion_matrix(obj_arr,n_test,11)
...
while True:
    ...
    #
    # losses3 = train_lstm(im_cond1,im_cond2,im_cond1,lb)
