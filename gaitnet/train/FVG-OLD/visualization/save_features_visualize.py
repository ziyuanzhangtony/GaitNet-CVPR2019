...
class FVG(object):
    ...
    def __init__(self,
        ...
        self.train_subjects = subjects[:136] #136 subjects
        self.test_subjects = subjects[136:136+10] #90 subjects
    ...
    def get_eval_data(self,
        ...
        # gallery = 2
        # # session1_probe = list(range(4, 9 + 1))
        # # session2_probe = list(range(4, 6 + 1))
        # session3_probe = []
        #
        # session1_probe = []
        # session2_probe = list(range(7, 9+1))
        ...
        test_data_glr = pad_sequences(test_data_glr,maxlen=150,dtype='float32',padding='post')
        test_data_prb = pad_sequences(test_data_prb,maxlen=150,dtype='float32',padding='post')
        ...
        def random_vi_idx(si):
            ...
            if si in list(range(1,147+1)):
                if si in [1,2,4,7,8,12,13,17,31,40,48,77]:
                    ...
                    reading_dir = 'SEG-S1'
                if self.video_i == []:
                    vi_idx = np.random.randint(1, 13)
                else:
                    vi_idx = np.random.choice(self.video_i)
            else:
                reading_dir = 'SEG-S2'
                if self.video_i == []:
                    vi_idx = np.random.randint(1, 10)
                else:
                    vi_idx = np.random.choice(self.video_i)
...
parser.add_argument('--siter', type=int, default=10000, help='number of itr to start with')
parser.add_argument('--lr', default=0.00001, type=float, help='learning rate')
parser.add_argument('--data_root',
                     default='/media/tony/STORAGE/DATA/FVG/',
...
time_now = '2018-11-22 16:49:25.953237'
...
def eval_lstm_roc(glr, prb, gt, n_test  = 10):
    ...
    return glr[-1],glr_vec,prb[-1],prb_vec,gt
...
def train_main(Xn, Xc, Xmx, l):
    ...
    loss_out_haha = mse_loss(hgs_n, hgs_c) / 100000
...
def save_data(glr, glr_vec ,prb, prb_vec , lb_gt ,itr,subdir):
    os.makedirs('data/glr/%s/' % (subdir),exist_ok=True)
    os.makedirs('data/prb/%s/' % (subdir),exist_ok=True)
    os.makedirs('data/glr/%s/%03d/'%(subdir,itr),exist_ok=True)
    os.makedirs('data/prb/%s/%03d' % (subdir,itr),exist_ok=True)
    c=0
    for i in range(10):
        imsave('data/glr/%s/%03d/%03d.png'%(subdir,itr,i),
               np.transpose(glr[i], (1,2,0))
               )
        if lb_gt[i] != 0:
            imsave('data/prb/%s/%03d/%03d.png'%(subdir,itr,i),
                   np.transpose(prb[c], (1,2,0))
                   )
            c+=1
    # with open('data/data_%s_%03d.pickle'%(subdir,itr), 'wb') as handle:
    # lst = [glr_vec,prb_vec,lb_gt]

    dict = {

        'glr_%s' % (subdir): glr_vec,
        'prb_%s' % (subdir): prb_vec,
        'gt_%s' % (subdir): lb_gt
    ...
    sio.savemat('data/data_%s_%03d.mat'%(subdir,itr), dict)


#################################################################################################################
# FUN TRAINING TIME !
# train_eval = test_data.get_eval_data(True)
proto1 = test_data.get_eval_data(False,2,session1_probe=[5])
proto2 = test_data.get_eval_data(False,2,session1_probe=[11])
proto3 = test_data.get_eval_data(False,2,session2_probe=[8])
proto4 = test_data.get_eval_data(False,2,session2_probe=[11])
# proto5 = test_data.get_eval_data(False,2,session3_probe=list(range(3, 12 + 1)),cross_session=True)
...
while True:
    ...
    if itr % 5 == 0:
        ...
        for i in range(5,150+1):
            ...
            scores1 = eval_lstm_roc(proto1[0][:i], proto1[1][:i], proto1[2])
            save_data(scores1[0],scores1[1],scores1[2],scores1[3],scores1[4],i,'WS')
            scores2 = eval_lstm_roc(proto2[0][:i], proto2[1][:i], proto2[2])
            save_data(scores2[0],scores2[1],scores2[2],scores1[3],scores2[4],i,'CB-OLD')

            scores3 = eval_lstm_roc(proto3[0][:i], proto3[1][:i], proto3[2])
            save_data(scores3[0],scores3[1],scores3[2],scores3[3],scores3[4],i,'CL')

            scores4 = eval_lstm_roc(proto4[0][:i], proto4[1][:i], proto4[2])
            save_data(scores4[0],scores4[1],scores4[2],scores4[3],scores4[4],i,'BG')

            # exit()


        # scores5 = eval_lstm_roc(proto1[0], proto1[1], proto1[2])
        # write_tfboard(scores1[:2], itr, name='SP')
        # write_tfboard(scores2[:2], itr, name='CB-OLD')
        # write_tfboard(scores3[:2], itr, name='CL')
        # write_tfboard(scores4[:2], itr, name='BG')
