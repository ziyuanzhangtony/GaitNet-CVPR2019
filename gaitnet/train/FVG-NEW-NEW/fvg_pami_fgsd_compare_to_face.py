...
from utils.modules.modules_fvg_pami import *
...
if not debug_mode:
    ...
    with torch.no_grad():
        ...
        # print(acc005_mean)
        all_scores = []
        proto_gait1 = test_data.get_eval_format_time()
        for i in range(1,11):
            p = i/10
            print(p)
            lengt = int(70 * p)
            glr = proto_gait1[0][:int(70 * p)]
            prb = proto_gait1[1][:int(70 * p)]

            eval_gait1 = eval_roc_two(glr, prb, proto_gait1[2], 6, [netE, lstm], opt)
            print(eval_gait1)
            all_scores.append(eval_gait1[0])
        print(all_scores)
