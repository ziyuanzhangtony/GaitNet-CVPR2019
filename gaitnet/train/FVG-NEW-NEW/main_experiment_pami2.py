...
from utils.modules_fvg_pami2_tab6 import *
...
parser.add_argument('--fgs_dim', type=int, default=16, help='size of the gait vector')
parser.add_argument('--fgd_dim', type=int, default=16, help='size of the gait vector')
parser.add_argument('--lstm_hidden_dim', type=int, default=128-16, help='size of the gait vector')
...
parser.add_argument('--signature', default='136training-MRCNN-DIFFCOND-fgs_fgd-DECAY-dynamic_the_last_output')
...
def train_main(Xn, Xc, l):
    ...
    for i in range(0, len(Xn)):
        ...
        fan0, (fgsn0, fgdn0) = netE(Xn[rdm])
        han1, (fgsn1, fgdn1) = netE(Xn[i])

        fac0, (fgsc0, fgdc0) = netE(Xc[rdm])
        hac1, (fgsc1, fgdc1) = netE(Xc[i])

        apperance_loss += mse_loss(fan0, han1)

        static_loss += mse_loss(fgsn0, fgsn1)
        static_loss += mse_loss(fgsn0, fgsc0)

        hgds_n.append(fgdn1)
        hgds_c.append(fgdc1)

        hgs_n.append((fgsn0, fgdn0))

    hgds_n = torch.stack(hgds_n)
    hgds_n_mean = torch.mean(hgds_n, 0)
    # hgds_n_mean = lstm(hgds_n, False)

    hgds_c = torch.stack(hgds_c)
    hgds_c_mean = torch.mean(hgds_c, 0)
    # hgds_c_mean = lstm(hgds_c, False)

    apperance_loss/=len(Xn)
    static_loss/=len(Xn)/2
    dynamic_loss = mse_loss(hgds_n_mean, hgds_c_mean)

    lstm_out_train, lstm_out_test, lstm_out_hidden = lstm(hgs_n)
    cse = cse_loss(lstm_out_train, l)

    loss = apperance_loss + 0.01 * dynamic_loss + 0.1* cse + static_loss
    # loss =  apperance_loss + dynamic_loss * 0.01 + cse * 0.1
    # loss = cse * 0.1
    # loss = apperance_loss + cse * 0.1 + static_loss
    ...
    return [apperance_loss.item(), dynamic_loss.item(), cse.item(), static_loss.item()]
